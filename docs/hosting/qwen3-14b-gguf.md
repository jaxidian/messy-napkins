...
This document only holds general, portable facts. Exact commands, exact live
evidence, and per-run reasoning for a specific hosted instance belong in the
markdown file paired with that config, e.g.
[configs/local/qwen3-14b-gguf.md](../../configs/local/qwen3-14b-gguf.md) (not
tracked in Git). Update this profile only when a re-check surfaces a new
*general* fact or caveat that should apply beyond one specific run.